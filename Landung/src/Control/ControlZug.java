...
public class ControlZug {
	...
	public boolean naechsterZug(String eingabe) {
		...
		boolean gueltigerZug = true;
		
		if (!istZugMoeglich() && ((2 < this.controlSpiel.getRundeZug() 
				&& this.controlSpiel.getRundeZug() < 4 
				|| this.controlSpiel.getRundeZug() > 4)
	...
	public boolean gewonnen() {
		...
		for (int k = 0; k < 5; k++) {
			for (int i = 0; i < 2; i++) {
				...
				for (int n = 0; n < 4; n++) {
					if (this.controlSpiel.getIstDran().getSymbol() == this.controlSpiel
					        .getSpielfeld().fetchSpielstein(pos).getSymbol()) {
						pos[1] += 1;
						anzahl++;
					}
				}
		...
		if (anzahl >= 4 || anzahl2 >= 4 || anzahl3 >= 4 || anzahl4 >= 4
		        || anzahl5 >= 4 || anzahl6 >= 4 || anzahl7 >= 4 || anzahl8 >= 4) {
			return true;
		} else {
			anzahl = 0;
			anzahl2 = 0;
	...
	public static boolean gueltigeEingabe(String eingabe) {
		...
		if (Pattern
		        .matches(
		                "[a-e][1-5]|[a-e][1-5][a-e][1-5]|[a-e][1-5][a-e][1-5][a-e][1-5]",
		                eingabe))
			return true;
	...
	public boolean sonderregel(int[] start, String eingabe) {
		if (eingabe.length() == 2) {
			...
			this.controlSpiel.getSpielfeld().setzeSpielstein(
			        this.controlSpiel.getIstDran().getSpielstein(), start);
			return true;
		}
		return false;
	}
	...
	public boolean ziehenSetzen(int[] start, int[] ziel, String eingabe) {
		if (eingabe.length() == 4) {
			...
			this.controlSpiel.getSpielfeld().setzeSpielstein(
			        this.controlSpiel.getIstDran().getSpielstein(), ziel);
			return true;
		}
		return false;
	}
